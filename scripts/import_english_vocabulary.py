"""
Import English vocabulary from JSON batch files in scripts/en/ into vocabulary_words.
...
def main() -> None:
    vocab_files = sorted(EN_DIR.glob("vocab_*.json"))
    if not vocab_files:
        print(f"ERROR: No vocab_*.json files found in {EN_DIR}")
        return

    conn = sqlite3.connect(str(DB_PATH))
    cur = conn.cursor()

    cur.execute("SELECT COUNT(*) FROM vocabulary_words WHERE language='en'")
    baseline = cur.fetchone()[0]
    print(f"English words before import: {baseline}")
    ...
    print(f"\nEnglish words before : {baseline}")
    print(f"English words after  : {final}")
    print(f"New words inserted   : {total_inserted}")

    print("\nBreakdown by CEFR level (English):")
    cur.execute("""
