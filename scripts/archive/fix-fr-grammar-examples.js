 ...
 *
 * Usage: node scripts/fix-fr-grammar-examples.js [--dry-run]
 */
...
function detectRuleMarker(structure) {
...
function getBaseExamplesForCategory(structure) {
  const label = structure.structure_label.toLowerCase();
  const pattern = (structure.pattern || '').toLowerCase();
  ...
  if (label.includes('combien')) {
    return [
      { fr: 'Combien coûte-t-il?', en: 'How much does it cost?' },
  ...
  if (label.includes('avoir') && !label.includes('au présent')) {
    return [
...
function generateExamplesForStructure(structure) {
...
function validateExample(example, structure) {
...
function grammarCheck(structure, examples) {
...
async function main() {
  ...
  console.log(`Examples inserted: ${inserted}`);
